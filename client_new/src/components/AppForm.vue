    ...
    <div v-if="formIsReady">
        ...
        <div class="d-flex justify-content-between">
            ...
            <div>
                ...
                <!-- show hidden fields -->
                <span class="ms-2" v-show="!hideForm" v-if="showDebugButtons" title="Show hidden fields" role="button" :class="{ 'text-success': !showHidden, 'text-danger': showHidden }" @click="showHidden = !showHidden">
                        ...
                        <div class="col py-0" v-if="visibility[field.name]" :class="field.width">
                            <div class="mt-3">
                                <div v-show="field.type != 'html'" class="d-flex">
                                    ...
                                    <!-- FIELD LABEL -->
                                    <label class="flex-grow-1 fw-bold mb-2" :class="{ 'text-body': !field.hide, 'text-grey': field.hide }">{{ field.label || field.name }} <span v-if="field.required" class="text-danger">*</span></label>

                                    <!-- FIELD DEBUG BUTTONS -->
                                    <div>
                                        <!-- refresh auto -->
                                        <BsButton @click="setFieldStatus(field.name, undefined)" v-if="fieldOptions[field.name] && fieldOptions[field.name].refresh" icon="arrow-rotate-right" :spin="true" cssClass="btn-sm">{{ fieldOptions[field.name].refresh }}</BsButton>
                                        <!-- refresh manual -->
                                        <span v-if="(field.expression != undefined || field.query != undefined) && field.refresh & !fieldOptions[field.name].refresh" role="button" class="me-2" @click="setFieldStatus(field.name, undefined)">
                                            <font-awesome-icon icon="arrow-rotate-right" />
                                        </span>
                                        <!-- enable field debug -->
                                        <span class="ms-2" role="button" :class="{ 'text-success': !fieldOptions[field.name].debug, 'text-danger': fieldOptions[field.name].debug }" @click="setExpressionFieldDebug(field.name, !fieldOptions[field.name].debug)" v-if="field.expression && showDebugButtons">
                                            <font-awesome-icon :icon="(fieldOptions[field.name].debug ? 'search-minus' : 'search-plus')" />
                                        </span>
                                        <!-- expression edit buttons -->
                                        <span class="ms-2 text-warning" role="button" @click="setExpressionFieldEditable(field.name, true)" v-if="field.editable && field.type == 'expression' && !fieldOptions[field.name].editable && !fieldOptions[field.name].viewable">
                                            <font-awesome-icon icon="pencil-alt" />
                                        </span>
                                        <span class="ms-2 text-danger" role="button" @click="setExpressionFieldEditable(field.name, false)" v-if="field.editable && field.type == 'expression' && fieldOptions[field.name].editable && !fieldOptions[field.name].viewable">
                                            <font-awesome-icon icon="times" />
                                        </span>
                                        <!-- raw content buttons -->
                                        <!-- show -->
                                        <span class="ms-2 text-success" role="button" @click="setExpressionFieldViewable(field.name, true)" v-if="showDebugButtons && field.expression && !fieldOptions[field.name].viewable && !fieldOptions[field.name].editable">
                                            <font-awesome-icon icon="eye" />
                                        </span>
                                        <!-- copy -->
                                        <span class="ms-2 text-info" role="button" @click="clip((field.type == 'expression') ? v$.form[field.name].$model : queryresults[field.name])" v-if="field.expression && fieldOptions[field.name].viewable && !fieldOptions[field.name].editable">
                                            <font-awesome-icon icon="copy" />
                                        </span>
                                        <!-- close -->
                                        <span class="ms-2 text-danger" role="button" @click="setExpressionFieldViewable(field.name, false)" v-if="field.expression && fieldOptions[field.name].viewable && !fieldOptions[field.name].editable">
                                            <font-awesome-icon icon="times" />
                                        </span>
                                    </div>
                                </div>
                                <!-- DEBUG EXPRESSION -->
                                <div class="mb-3" @dblclick="clip(field.expression, true)" v-if="field.expression && fieldOptions[field.name].debug">
                                    <pre v-highlightjs><code language="javascript">{{ field.expression }}</code></pre>
                                </div>
                                <!-- DEBUG EVALUATION -->
                                <div class="mb-3" @dblclick="clip(fieldOptions[field.name].expressionEval, true)" v-if="field.expression && fieldOptions[field.name].debug && dynamicFieldStatus[field.name] != 'fixed'">
                                ...
                                <div v-if="field.type == 'enum'">
                                    <BsInputForForm type="select" :containerSize="containerSize" v-show="!fieldOptions[field.name].viewable" :defaultValue="defaults[field.name]" :required="field.required || false" :multiple="field.multiple || false" :name="field.name" :placeholder="field.placeholder || 'Select...'" :values="field.values || queryresults[field.name] || []" :hasError="v$.form[field.name].$invalid" :isLoading="!field.values && !['fixed', 'variable'].includes(dynamicFieldStatus[field.name])" v-model="v$.form[field.name].$model" :icon="field.icon" :columns="field.columns || []" :pctColumns="field.pctColumns || []" :filterColumns="field.filterColumns || []" :previewColumn="field.previewColumn || ''" :valueColumn="field.valueColumn || ''" @input="evaluateDynamicFields(field.name)" :sticky="field.sticky || false" :horizontal="field.horizontal || false" :errors="v$.form[field.name].$errors" />
                                    <!-- raw query data -->
                                    <div @dblclick="setExpressionFieldViewable(field.name, false)" v-if="fieldOptions[field.name].viewable" class="card p-2 limit-height is-limited">
                                ...
                                <!-- TYPE = DATEPICKER -->
                                 <div v-if="field.type=='datetime'">
                                    <BsInputForForm type="datetime" :icon="field.icon" v-model="v$.form[field.name].$model" :name="field.name" :hasError="v$.form[field.name].$invalid" :dateType="field.dateType" @change="evaluateDynamicFields(field.name)" />
                                 </div>

                                <!-- TYPE = EXPRESSION -->
                                <div v-if="field.type == 'expression'" :class="{ 'is-loading': (dynamicFieldStatus[field.name] == undefined || dynamicFieldStatus[field.name] == 'running') & !fieldOptions[field.name].editable }">
                                    <div v-if="!fieldOptions[field.name].viewable">
                                        <BsInputForForm v-if="fieldOptions[field.name].editable" :icon="field.icon" type="text" @focus="preventFocus" :hasError="v$.form[field.name].$invalid" v-model="v$.form[field.name].$model" :name="field.name" :required="field.required" @change="evaluateDynamicFields(field.name)"  :errors="v$.form[field.name].$errors" />
                                ...
                                <BsInputForForm v-if="['text','password','textarea','number','checkbox','radio'].includes(field.type)" @focus="preventFocus" @keydown="(field.keydown) ? evaluateDynamicFields(field.name) : null" @change="evaluateDynamicFields(field.name)" :hasError="v$.form[field.name].$invalid" v-model="v$.form[field.name].$model" :name="field.name" v-bind="field.attrs" :required="field.required" :type="field.type" :icon="field.icon" :readonly="field.hide" :placeholder="field.placeholder" :isSwitch="field.switch" :errors="v$.form[field.name].$errors" :values="field.values" />
                                                                
                                <!-- TYPE = FILE -->
                                <BsInputForForm v-if="field.type == 'file'" :accept="(field.accept||[]).join(',')" :type="field.type" @change="handleFiles" :hasError="v$.form[field.name].$invalid" :name="field.name" :required="field.required" :icon="field.icon" :errors="v$.form[field.name].$errors" :progress="fileProgress[field.name]" />
                            </div>
                        </div>
        ...
        <div class="d-grid mt-3" v-if="status==''">
            <button type="button" class="btn text-white btn-primary" @click="status='initializing'"><FaIcon :icon="submitIcon"></FaIcon><span class="ms-3">{{ submitLabel }}</span></button>
        </div>                
    </div>
