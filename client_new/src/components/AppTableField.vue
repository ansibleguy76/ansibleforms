        ...
        <BsOffCanvas  :show="showEdit" :title="action" @close="closeOffCanvas()">
            ...
            <template #default v-if="!!v$?.editedItem?.$model">
                ...
                <div v-for="field,index in editFields" :key="field.name" class="mt-3">
                    ...
                    <BsInputForForm v-if="['text','password','textarea','number','checkbox','radio'].includes(field.type)" :autofocus="index==0" :hasError="v$.editedItem[field.name].$invalid" v-model="v$.editedItem[field.name].$model" :name="field.name" v-bind="field.attrs" :required="field.required" :type="field.type" :icon="field.icon" :readonly="field.hide" :placeholder="field.placeholder" :isSwitch="field.switch" :errors="v$.editedItem[field.name].$errors" :values="field.values" />
        ...
        <table :class="getTableClass">
          ...
          <tbody>
              <template v-if="!isLoading">
                <tr class="is-unselectable" v-for="row,index in rows" :key="'table-row-' + index" @click="rowClick(row)">
                  <template v-if="deleteMarker && row[deleteMarker]">
                    <td>
                      ...
                      <span class="me-2 text-success" role="button" @click="undoRemoveItem(index)"><font-awesome-icon icon="undo" /></span>
                    </td>
                    <slot name="table-body" :row="row">
                        <template v-for="field in tableFields"  :key="'table-cell-' + field.name + '-' + index">
                          <td class="text-secondary" v-if="field.type!='checkbox'" :class="field.bodyClass"> {{ stringify(row[field.name],field) }} </td>
                          <td class="text-secondary" v-else :class="field.bodyClass"><font-awesome-icon :icon="(row[field.name])?['far','check-square']:['far','square']" /></td>
                        </template>
                    </slot>
                  </template>
                  <template v-else>
                    <td>
                      <span role="button" class="me-2 text-success" v-if="allowInsert" @click="addItem(index)"><font-awesome-icon icon="plus-square" /></span>
                      <span role="button" class="me-2 text-orange" @click="editItem(index)"><font-awesome-icon icon="pencil-alt" /></span>
                      <span role="button" class="me-2 text-danger" v-if="allowDelete || row[insert_marker]" @click="removeItem(index)"><font-awesome-icon icon="times" /></span>
                      <span role="button" class="me-2 text-secondary" v-else><font-awesome-icon icon="times" /></span>
                    </td>
                    <slot name="table-body" :row="row">
                        <template v-for="field in tableFields" :key="'table-cell-' + field.name + '-' + index">
                          <td v-if="field.type!='checkbox'" :class="field.bodyClass"> {{ stringify(row[field.name],field) }} </td>
                          <td v-else  :class="field.bodyClass"><font-awesome-icon :icon="(row[field.name])?['far','check-square']:['far','square']" /></td>
                        </template>
                    </slot>
                  </template>
                </tr>
                <tr v-if="allowInsert">
                    <td>
                      <span role="button" class="me-2 text-success" @click="addItem(-1)"><font-awesome-icon icon="plus-square" /></span>
                    </td>

                    <slot name="table-body">
                       <td v-for="field in tableFields" :key="'table-cell-' + field.name" class="bg-secondary-subtle" ></td>
                    </slot>
                </tr>
              </template>
              <template v-else>
