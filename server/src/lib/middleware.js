...
Middleware.checkAdminMiddleware = (req, res, next) =>  {
  try{
    if(!req.user.user.roles.includes("admin")) {
      res.status(401).json(RestResult.error("No access","You are not an admin"))
    } else {
...
Middleware.checkSettingsMiddleware = (req, res, next) => {
  try {
    if (!req.user.user.options.showSettings) {
      res.status(401).json(RestResult.error("No access","You do not have access to settings"));
    } else {
      next();
    }
  } catch (e) {
...
Middleware.checkDesignerMiddleware = (req, res, next) => {
  try {
    if (!req.user.user.options.showDesigner) {
      res.status(401).json(RestResult.error("No access","You do not have access to designer"));
    } else {
      next();
    }
  } catch (e) {
...
Middleware.checkLogsMiddleware = (req, res, next) => {
  try {
    if (!req.user.user.options.showLogs) {
      res.status(401).json(RestResult.error("No access","You do not have access to logs"));
    } else {
      next();
    }
  } catch (e) {
...
Middleware.checkBackupMiddleware = (req, res, next) => {
  try {
    if (!req.user.user.options.allowBackupOps) {
      res.status(401).json(RestResult.error("No access", "You do not have access to database operations"));
    } else {
      next();
    }
  } catch (e) {
...
Middleware.checkScheduledJobsMiddleware = (req, res, next) => {
  try {
    if (!req.user.user.options.allowScheduledJobs) {
      res.status(401).json(RestResult.error("No access", "You do not have permission to manage scheduled jobs"));
    } else {
      next();
    }
  } catch (e) {
...
Middleware.checkStoredJobsMiddleware = (req, res, next) => {
  try {
    if (!req.user.user.options.allowStoredJobs) {
      res.status(401).json(RestResult.error("No access", "You do not have permission to manage stored jobs"));
    } else {
      next();
    }
  } catch (e) {
